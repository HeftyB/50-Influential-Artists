...
function getArtistByIndex(artAry, index) {
    return `The artist at index ${index} is ${artAry[index].name}`;
}
...
function removeArtist (artAry, index) {
    delete artAry[index];
  }
...
//Instructions do not make sense, the data set only contains a year range and not a date of birth, so I assume all year ranges in "years" start with the year they were born? 
//I was able to complete the problem, but the dataset or intructions should clarify on the data stored or provide better instructions on how they would like problem solved.
//Also the 20th century is 1901-2000

function get20s(artAry){

  let newArray = [];
  
  for (i = 0; i < artAry.length; i++) {
    let newStringArray = artAry[i].years.split("", 4);
    let birthYear = newStringArray.join("");
    let newInt = parseInt(birthYear, 10);
    
    if (newInt > 1800 && newInt < 1900) {
      newArray.push(artAry[i].name);
    }
  }
